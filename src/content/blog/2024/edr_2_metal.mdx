...
I couldn't get perceptual quantization working properly, so I can't go into
much depth. It is used by HDR10 and has a massive maximum luminance of
...
If there is headroom, you can enable EDR rendering by setting
...
This of course, means that using EDR to boost the maximum brightness does not
work well. Unfortunately, I didn't realize this until pretty late into development
...
The `extendedLinear*` color spaces map `[0, 1]` to SDR content while anything
beyond that was in HDR land. As such, normalized integer pixel formats can't
support these linear color spaces.

Quantization error is not an issue (at least when compared to standard 8-bit
SDR formats): one good thing about floating point formats is that accuracy is high
for small values and decreases with magnitude, mirroring the behavior of
...
iOS can do this automatically by setting the `CAMetalLayer.edrMetadata` property.
...
Even if I set `edrMetadata` to `nil`, PQ directly outputs luminance values,
not brightness relative to SDR white. As there doesn't seem to be any
way of determining the current display luminance (for either SDR or HDR white
under the current headroom), I gave up on working on this.
